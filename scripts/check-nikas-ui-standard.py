...
def main() -> None:
    ...
    if role == "specialized":
        ...
        source_kit_digest = hashlib.sha256(source_kit_text.encode("utf-8")).hexdigest()
        require(source_kit_digest == source_kit.get("sha256"), "canonical shell source-kit hash drift")
        require(source_kit["path"] in config.get("build_source_files", []), "shell kit must be a declared build input")
        require(source_kit_text in sources, "production bundle must contain the exact vendored shell source")

        peer_selector = config.get("peer_selector", {})
        require(peer_selector.get("present") is False, "LIDER must not expose a peer-device selector")
        require(
            peer_selector.get("reason") == "single_three_phase_device",
            "L1/L2/L3 must remain phases of one LIDER device",
        )
