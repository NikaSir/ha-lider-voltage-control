...
### LIDER approved Header-return extension

For LIDER, the centered two-line title is the explicitly approved parent-navigation surface and remains a persistent 44 px+ button. It shows `LIDER` and `UI vX.Y.Z`, contains no arrow or separate `Назад` label, and returns to the captured NikaS base panel through explicit Home Assistant navigation (`history.pushState()` plus `location-changed`). `history.back()` is prohibited.

The source route is captured once in this order: `return_to`/`from`, one-shot `nikas.specialized.source_route.v1`, the saved LIDER route, a safe same-origin referrer, configured `parent_route`, then `/dashboard-infrastructure/overview`. Only `/dashboard-house`, `/dashboard-actions` and `/dashboard-infrastructure` routes are accepted. Telemetry updates never replace the Header or recalculate its route.
